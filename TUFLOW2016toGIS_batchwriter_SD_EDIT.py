...
TUFLOWGIS = "C:\\TuflowGIS\\w64\\TUFLOW_to_GIS_w64.exe"#gets path of TUFLOW to GIS.exe from user
...
for filename in glob.glob('*.xmdf'):#defines new variable 'filename'loops through workingfolder and adds each 'filename' with .dat in it to listB
    print filename
    inputlistB.append(filename)
...
for filename in inputlistB: # creates new varible 'inputfile' from each item in list inputlistB
    line = TUFLOWGIS+' -b'+' -asc'+' -max'+' -typed '+filename
    linestr = str(line)
